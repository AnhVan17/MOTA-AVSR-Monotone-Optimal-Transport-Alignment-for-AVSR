...
def unpack_selective(train_limit: int = 50, clean_first: bool = False):
    ...
    for idx, tar_path in enumerate(all_targets):
        ...
        try:
            # This preserves train/test split information in the path
            tar_subfolder = os.path.join(RAW_OUTPUT_DIR, os.path.splitext(fname)[0])
            os.makedirs(tar_subfolder, exist_ok=True)
            
            with tarfile.open(tar_path, 'r') as tar:
                tar.extractall(path=tar_subfolder)
            success_count += 1
...
def main(train_limit: int = 20, clean_first: bool = False):
