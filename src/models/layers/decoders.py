...
class HybridDecoder(nn.Module):
    ...
    def forward(
        ...
        if target is not None:
            ...
            # CRITICAL FIX: Replace -100 (ignore_index) with 0 before embedding
            # This prevents CUDA index error while still allowing Loss to use -100 for masking
            target_for_embed = target.clone()
            target_for_embed[target_for_embed < 0] = 0  # Replace all negative values
            target_for_embed[target_for_embed >= self.vocab_size] = 0  # Also clamp too large values
